...
current_player = "X"
board = [""] * 9
state_space = []  # Stores board states
...
def on_click(index):
    global current_player, state_space

    if board[index] == "" and not check_winner():  # Ensure the cell is empty
        board[index] = current_player
        state_space.append(board.copy())  # Store the state
...
def reset_game():
    global current_player, board, state_space
    current_player = "X"
    board = [""] * 9
    state_space = []  # Reset state space
